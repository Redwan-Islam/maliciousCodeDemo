...
def contains_malicious_patterns(file_content):
    ...
    malicious_patterns = [
        r"os\.system\(",  # Matches os.system(...)
        r"with open\(",   # Matches with open(...)
        r"eval\(",        # Matches eval(...)
        r"exec\(",        # Matches exec(...)
        r"import subprocess",  # Matches import subprocess
        r"import os",      # Matches import os
    ]
            ...
            if item['type'] == 'blob' and item['path'].endswith(('.py', '.js', '.php', '.rb', '.sh', '.txt', '.html')):
                ...
                for line in file_content.splitlines():
                    ...
                    if contains_malicious_patterns(line):
                        logging.info(f"Malicious pattern found in {file_name} at line {line_number}: {line}")
                        ...
                        logging.info(f"No malicious pattern in {file_name} at line {line_number}: {line}")
